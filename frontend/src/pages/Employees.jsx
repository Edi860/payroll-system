...
const Employees = () => {
  ...
  const handleEdit = () => {
    ...
    setEmployees(employees.map((e) =>
      e.id === selected.id ? { ...e, ...formData, salary: parseFloat(formData.salary) || e.salary } : e
    ));
  ...
  const handleSalaryUpdate = () => {
    ...
    setEmployees(employees.map((e) =>
      e.id === selected.id ? { ...e, salary: parseFloat(salaryAmount) } : e
    ));
    showToast(`💰 Salary updated to $${parseFloat(salaryAmount).toLocaleString()}`);
    closeModal();
  };
  ...
  const toggleStatus = (emp) => {
    ...
    showToast(`Status updated for ${emp.name}`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Toast Notification */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 rounded-xl bg-slate-800 px-5 py-3 text-sm font-medium text-white shadow-xl">
          {toast}
        </div>
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {isHR && (
          <button
            onClick={() => { setFormData(EMPTY); setModal('add'); }}
            className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:opacity-90 transition shadow"
          >
            + Add Employee
          </button>
        )}
              ...
              <div className="p-5">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${avatarColors[i % avatarColors.length]} text-white font-bold text-lg`}>
                      {emp.avatar}
                    </div>
                    <div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">
                  ...
                  {isHR && (
                    <button
                      onClick={() => { setSelected(emp); setFormData({ name: emp.name, email: emp.email, phone: emp.phone, department: emp.department, role: emp.role, salary: emp.salary, status: emp.status, joinDate: emp.joinDate }); setModal('edit'); }}
                      className="rounded-lg bg-blue-50 px-2 py-2 text-xs font-semibold text-blue-600 hover:bg-blue-100 transition"
                    >
                      ✏️ Edit
                    </button>
                  )}
                  ...
                  {isHR && (
                    <button
                      onClick={() => { setSelected(emp); setSalaryAmount(emp.salary); setModal('salary'); }}
                      className="rounded-lg bg-green-50 px-2 py-2 text-xs font-semibold text-green-600 hover:bg-green-100 transition"
                    >
                      💰 Salary
                    </button>
                  ...
                  {isHR && (
                    <button
                      ...
                      className="rounded-lg bg-purple-50 px-2 py-2 text-xs font-semibold text-purple-600 hover:bg-purple-100 transition"
                    >
                      💵 Advance
                    </button>
                  )}

                  {/* HR + Admin can Deactivate */}
                  {isHR && (
                    <button
                      onClick={() => toggleStatus(emp)}
                      className={`rounded-lg px-2 py-2 text-xs font-semibold transition ${emp.status === 'Active' ? 'bg-amber-50 text-amber-600 hover:bg-amber-100' : 'bg-green-50 text-green-600 hover:bg-green-100'}`}
                    >
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="rounded-lg bg-red-50 px-2 py-2 text-xs font-semibold text-red-600 hover:bg-red-100 transition"
                    >
                      🗑️ Delete
                    </button>
                  )}
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl overflow-hidden">
            ...
            <div className="p-6 space-y-2">
              {[
                ...
                { label: '💰 Salary', value: `$${selected.salary.toLocaleString()}/month` },
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl p-6 max-h-screen overflow-y-auto">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Role', key: 'role', type: 'text', placeholder: 'e.g. Developer' },
                { label: 'Salary ($)', key: 'salary', type: 'number', placeholder: 'e.g. 3000' },
                { label: 'Join Date', key: 'joinDate', type: 'date', placeholder: '' },
              ].map(({ label, key, type, placeholder }) => (
                <div key={key}>
                  <label className="text-xs font-medium text-slate-500">{label}</label>
                  <input type={type} className={inputClass} value={formData[key]} placeholder={placeholder} onChange={(e) => setFormData({ ...formData, [key]: e.target.value })} />
                </div>
              ))}
              <div>
      ...
      {modal === 'advance' && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl p-6">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs font-medium text-slate-500">Advance Amount ($)</label>
                <input type="number" className={inputClass} value={advanceAmount} onChange={(e) => setAdvanceAmount(e.target.value)} placeholder="e.g. 500" />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-500">Reason</label>
                <textarea className={`${inputClass} resize-none`} rows={3} value={advanceReason} onChange={(e) => setAdvanceReason(e.target.value)} placeholder="Reason for advance payment..." />
              </div>
            </div>
            <div className="flex gap-3 mt-4">
